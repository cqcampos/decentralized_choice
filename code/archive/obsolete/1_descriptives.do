...
program define whoApplies 
	...
	********************************** Make a table *****************************************************	
	texdoc init $tables/descriptives.tex, replace force
	...
	*local panela_vars "appliedMagnet magnet_offer"
	...
	texdoc write \multicolumn{4}{l}{\textbf{Panel B: Student Demographics}} \\
	texdoc write & & & \\
	...
	foreach var of local demo_vars{
		...
		*sum `var' if appliedAny==1 & e(sample)==1
		...
		texdoc write `rowname' & `mu_all' & `mu_m' &  `d'`d_star' \\
		texdoc write  &  &  & (`d_se') \\
	}	
	 
	texdoc write \multicolumn{4}{l}{\textbf{Panel C: Standardized Test scores}} \\
	texdoc write & & & \\
	...
	texdoc write \multicolumn{4}{l}{\textbf{Panel D: Distance to Schools}} \\
	texdoc write & & & \\
	...
	foreach var of local dist_vars{
		...
		if "`var'"=="rel_nearest_choice_dist" local rowname "Relative Distance to Nearest Choice School"
	...
	texdoc write Observations & `nonMagStu' & `magStu' & `allStu' \\
