...
program define whoApplies 
	...
	********************************** Make a table *****************************************************	
	...
	texdoc write & Non-Applicants & Applicants & Difference & Enrollees \\
	texdoc write  & (1) & (2) & (3) & (4) \\ \hline
	texdoc write & & & \\
	...
	*/
	...
	foreach var of local demo_vars {
		...
		*sum `var' if appliedAny==1 & e(sample)==1
		...
		texdoc write `rowname' & `mu_all' & `mu_m' &  `d'`d_star' & `mu_e' \\
		texdoc write  &  &  & (`d_se') & \\
	}	
	...
	count if next_schoice ==1 
	local EnrStu = r(N)

	texdoc write Observations & `nonMagStu' & `magStu' & `allStu' & `EnrStu' \\
