...
if (homog == 0){
  ...
  if (eta_out){
    ...
    omega_choice_0 <- c(
      ...
      -1.5609554828, 
      -0.2373604967, -0.1076556340, 0,#ln(sd(theta 1)), ln(sd(theta 2)) ln(sd(theta 3))
      2.5353127870, 0, # alpha_1
      -0.2317632725, 0 # mu_1
    )
    ...
    omega_choice_0 <- c(
      ...
      0.480225128,  
      0.501874224,  0.586712173, 0, #ln(sd(theta 1)), ln(sd(theta 2)), ln(sd(theta 3))
      2.337216832, 0, # alpha_1, alpha_2
      -0.670004060, 0 # mu_1, mu_2
    )
