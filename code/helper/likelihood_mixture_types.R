likelihood_K_mixture <- function(omega, data, grad = FALSE, log_file=NULL, posterior=FALSE) {
  ...
  l_and_grad_r <- function(r, k){
    ...
    theta <- mu_theta[k] + sigma_theta[k] * sims_theta_r
    eta_ij <- sigma_c * sims_c[,,r]  
    
    if(posterior){
      log_prior_draw <- -0.5 * (sims_theta_r^2 )    # N×1
      theta_draw <- theta 
    }
    ...
    #if(posterior){
    #  log_prior_r[,r,k]  <- -0.5 * (sims_theta_r^2) 
    #  theta_r[,r,k] <- theta 
    #}
    ...
    if (grad){
      ...
      return(list(likelihood_r=likelihood_r, log_prior_r = log_prior_draw, theta_r=theta_draw ))
    }
  ...
  if(posterior){
    log_prior_k <- array(0, dim = c(N, R_draws, K))
    theta_k <- array(0, dim = c(N, R_draws, K))
    posterior_theta_mu <- array(0, dim = c(N, K))
    posterior_theta_var <- array(0, dim = c(N, K))
  }
  for (k in 1:K){
    ...
    if (grad){
      ...
      likelihood_sim <- do.call(cbind, lapply(res, `[[`, 1))
      log_prior_mat  <- do.call(cbind, lapply(res, `[[`, "log_prior_r"))  # N × R
      theta_mat      <- do.call(cbind, lapply(res, `[[`, "theta_r"    ))  # N × R
    }
    ...
    theta_posterior_var <- NULL
    
    # Calculate posterior means 
    if(posterior==TRUE){
      print("Calculating posterior means and variances...")
      #log_prior_r[ , , k] <- log_prior_mat
      #theta_r[ , , k] <- theta_mat
      ...
      posterior_theta_mu[,k] <- rowSums(weights * theta_mat)
      posterior_theta_var[,k] <- rowSums(weights * (theta_mat - posterior_theta_mu[,k])^2 )
    }
  }
  ...
  theta_posterior_var <- NULL
  if(posterior){
    post_mix_weights <- likelihood_i_k * matrix(p_theta,
                                                nrow = nrow(likelihood_i_k),
                                                ncol = ncol(likelihood_i_k),
                                                byrow = TRUE)
    post_mix_weights <- post_mix_weights / rowSums(post_mix_weights) 
    #theta_posterior_mu <- posterior_theta_mu %*% matrix(p_theta, nrow=K, ncol = 1)
    #theta_posterior_var<- posterior_theta_var %*% matrix(p_theta, nrow=K, ncol = 1)
    theta_posterior_mu <- rowSums(post_mix_weights * posterior_theta_mu)
    theta_posterior_var <- rowSums(post_mix_weights * posterior_theta_var)
  }
  ...
  return(list(L = likelihood_i, G = G_i, 
              ...
              posterior_var = theta_posterior_var))
}
